...
class IKTest(object):
    ...
    def __init__(self):
		...
		self.IKSolver = rospy.ServiceProxy('IK_service', robot_IK)
    ...
    def run_tests(self):
		...
		response_data = self.IKSolver(self.IK_REQ)

		errors += self.verify_result(response_data)
    ...
    def verify_result(self,IK_resp):
		...
		for x in xrange(0, len(IK_resp.points)): 
			...
			theta_r = IK_resp.points[x].positions # something
			theta_e = self.IK_RESP[x].positions# something
